...
total_input=$(awk -v a="$vout0_amount" -v b="$vout1_amount" 'BEGIN {print a + b}')


new_tx=$(bitcoin-cli -regtest createrawtransaction '[{"txid": "'$utxo_txid_1'", "vout": 0, "sequence": 1}, {"txid": "'$utxo_txid_1'", "vout": 1, "sequence": 1}]' '{"'$recipient'": 0.2}')
