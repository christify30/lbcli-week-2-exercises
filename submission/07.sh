# Create a raw transaction with an amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
changeaddress=$(bitcoin-cli -regtest getrawchangeaddress legacy)
recipient="2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP"

utxo_txid_1=$(bitcoin-cli -regtest listunspent | jq -r '.[0] | .txid')
utxo_vout_1=$(bitcoin-cli -regtest listunspent | jq -r '.[0] | .vout')
utxo_txid_2=$(bitcoin-cli -regtest listunspent | jq -r '.[2] | .txid')
utxo_vout_2=$(bitcoin-cli -regtest listunspent | jq -r '.[2] | .vout')

raw_tx=$(bitcoin-cli -regtest -named createrawtransaction inputs='''[ { "txid": "'$utxo_txid_1'", "vout": '$utxo_vout_1' }, { "txid": "'$utxo_txid_2'", "vout": '$utxo_vout_2' } ]''' outputs='''{ "'$recipient'": 0.2, "'$changeaddress'": 0.03669108 }''')

echo "$raw_tx"
#raw_tx=""
